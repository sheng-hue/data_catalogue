...
def build_llm_prompt(columns_batch: pd.DataFrame,
    ...
    prompt_parts = [
        ...
        "\n- Infer the PURPOSE and MEANING of each column from its name, type, and sample values",
        "\n- Describe WHAT the field represents, not just what values it contains",
        "\n- DO NOT simply list or repeat the sample values in the description",
        "\n- If you cannot determine the purpose, write: 'Unable to determine purpose from available data' (confidence: 1)",
        "\n- Use 'Likely' or 'Possibly' prefixes only when you have a reasonable hypothesis (confidence: 2-3)",
        "\n- Confidence scoring: 5=certain from context, 4=high confidence, 3=medium confidence, 2=low confidence/guess, 1=unknown/unable to determine",
